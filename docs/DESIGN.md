...
## NeuroGuard Adaptations

This system is applied to NeuroGuard with two documented deviations — a health
screening app is not a marketing site:

1. **Semantic risk colors are retained.** The reference system is achromatic,
   but Low/Borderline/Elevated verdicts, probability bars, badges and the risk
   gauge need green/amber/red. Dark-adapted values: low `#34d399`, borderline
   `#fbbf24`, elevated `#f87171`, with ~13%-alpha soft washes; text-on-wash
   variants (`#6ee7b7` / `#fcd34d` / `#fca5a5`) clear WCAG 4.5:1 on graphite.
2. **Hairlines run translucent.** A literal 1px solid `#e5e5e5` on `#0a0a0a`
   reads as a glowing wire; NeuroGuard uses `rgba(229,229,229,0.14)` (0.24 on
   hover/strong) to match how the reference sites actually render.

Implementation lives entirely in `frontend/src/index.css` token block +
component classes; fonts load in `frontend/index.html`. The hero gradient
(amber → cobalt) appears only on the auth pages; the dusk-violet wash appears
only under the active nav pill and in focus/selection washes — never as solid
fills.
